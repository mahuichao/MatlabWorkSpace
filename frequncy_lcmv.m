...
% audiowrite('audio1.wav',x1,fs);
% audiowrite('audio2.wav',x2,fs);
% audiowrite('audio3.wav',x3,fs);
% audiowrite('audio4.wav',x4,fs);
% audiowrite('audio5.wav',x5,fs);
% audiowrite('audio6.wav',x6,fs);


x1=audioread('audio1.wav');
x2=audioread('audio2.wav');
x3=audioread('audio3.wav');
x4=audioread('audio4.wav');
x5=audioread('audio5.wav');
x6=audioread('audio6.wav');
...
%% 初始化
c=340;
desire_angle=90; % 期望角度   这里有个细节，这里的0度对应模拟ULA的90度
NFFT=1024; % 帧长1024
Overlap=NFFT/2;
MicNum=6; % 6个麦克风
...
n_blocks=fix((size(X,2)-Overlap)/Overlap);
...
name=['result_' num2str(mu) '_' num2str(desire_angle) '.wav']
